...
# 2. 本番 HTML にキーワードが含まれるか確認
curl -s https://astro-attendsalon-r.pages.dev/ | grep -o "NO IMAGE\|voice-01"
```

### コミットメッセージ規約

- **1行のみ**（改行禁止）— Cloudflare Pages API が改行入りメッセージをエラーにする（code: 8000111）
- `type(scope): description` 形式を使う（例: `feat(hero): add placeholder image`）
- `git commit -m "$(cat <<'EOF'...)"` のヒアドキュメント形式は**使わない**
- push 後にメッセージを直したい場合は `git revert` または新規コミットで対処（`--amend` + `force-push` はしない）
