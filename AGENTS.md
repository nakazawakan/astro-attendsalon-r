...
~/sites/astro-attendsalon-r/   （Cursor ワークスペース例: /Users/.../sites/astro-attendsalon-r と同じリポジトリ）
  src/
    ...
    styles/        現在は soft-motion.css。新規スタイルは foundation / layout / components / utilities と各 .astro scoped CSS へ整理
  public/
...
|------|------------------------|-----------------------------|----------|------|
| 本文最大幅 | `--page-max-width` / `.l-page-inner` | `max-w-page` | `960px` | シェル**内側**で使うブロック（見出し下説明、カード列など）の上限の目安 |
| 左右ガター | `--page-gutter` / `.l-page-shell` | `px-6` | `24px` × 左右 | **全ブレークポイントで常時**（`md:px-0` 等で消さない） |
| 本文シェル幅 | `--page-shell-max-width` / `.l-page-shell` | `max-w-pageShell` | `1008px` | **960 + 24 + 24**。シェル要素の `max-width` の上限 |

式: **`pageShell = page + gutter×2`** → `1008 = 960 + 24 + 24`。

### ボックスモデル（重要）

`Foundation reset` により要素は原則 **`box-sizing: border-box`** とする。本文シェルは次を正とする。
...
- `.l-page-shell` … ブロック全体の幅の上限 + 左右ガター
- `.l-page-inner` … シェル内部の本文最大幅
...
3. **内側でさらに狭めたい**ときだけ `.l-page-inner` の子に別クラスを追加する。

### 禁止・要レビュー

- **PC（`md:` 以上）**で、親のガターを打ち消す `margin-inline: -24px` + `width: calc(100% + 48px)` などのフルブリード（シェル外へはみ出し）。`html` / `body` の `overflow-x-clip`（`BaseLayout`）と組み合わせると、**左だけ欠けて非対称**になることがある。
...
8. モバイル対応は `md = 768px` を基準に設計（デスクトップは 1440px 基準）。**本文シェル・ガターは上記「レイアウト仕様書（本文シェル）」に従う**。`BaseLayout` の `body` に `overflow-x-clip` あり
